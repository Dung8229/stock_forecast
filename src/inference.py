...
def load_inference_data(stock_list=STOCK_LIST, start_day=YESTERDAY) -> Dict:
    logger.info(f"Loading stock data at {start_day} for {len(stock_list)} stocks.")
    df_all = yf.download(stock_list, start=start_day, group_by="ticker", auto_adjust=True)
    all_data = {}
...
"""
    create_markdown_artifact(
        key=f"raw-inference-data-summary",
        markdown=markdown,
    ...
    for ticker, df in all_data.items():
        ...
        logger.info(f"Uploaded {ticker} data to S3: {INFER_RAW_DIR}/{ticker}.csv")
    return all_data

@materialize("s3://zoomcamps-bucket/data/inference/processed")
def export_processed_inference_data(df_infer=pd.DataFrame, export_dir=INFER_PROCESSED_DIR):
