...
import joblib
from src.utils.s3_io import upload_df_to_s3, download_df_from_s3, upload_joblib_to_s3
...
def load_data(stock_list=STOCK_LIST, start=START_DATE, end=END_DATE) -> Dict:
    logger.info(f"Loading stock data from {start} to {end} for {len(stock_list)} stocks.")
    df_all = yf.download(stock_list, start=start, end=end, group_by="ticker", auto_adjust=True)
    all_data = {}
...
"""
    ...
    for ticker, df in all_data.items():
        ...
        logger.info(f"Uploaded {ticker} data to S3: {RAW_EXPORT_DIR}/{ticker}.csv")
    return all_data
...
def process_data(
    ...
    if not for_inference:
        ...
        return df_train.reset_index(drop=True), df_test.reset_index(drop=True), encoder
